...
COMMENT ON COLUMN audit.logged_actions.application_user IS 'Application user set when this audit event occurred. Can be changed in-session by client.';
COMMENT ON COLUMN audit.logged_actions.action IS 'Action type; I = insert, D = delete, U = update, T = truncate';
COMMENT ON COLUMN audit.logged_actions.row_data IS 'Record value. Null for statement-level trigger. For INSERT this is the new tuple. For DELETE and UPDATE it is the old tuple.';
COMMENT ON COLUMN audit.logged_actions.row_id IS 'Record id. Null if an id field does not exist.';
...
CREATE INDEX logged_actions_row_id_idx on audit.logged_actions(row_id);
...
BEGIN
    ...
    audit_row = ROW(
        ...
        current_setting('application_name'),         				-- client application
				current_setting('application_session.user',true),  	-- client application user
        inet_client_addr(),    				                      -- client_addr
        inet_client_port(),				                          -- client_port
        current_query(),          			                    -- top-level query or queries (if multistatement) from client
        substring(TG_OP,1,1),           			              -- action
        NULL, NULL,NULL,                      			        -- row_data, row_id, changed_fields
        'f'                                         			  -- statement_only
        );
    ...
    IF (TG_OP = 'UPDATE' AND TG_LEVEL = 'ROW') THEN
		...
		IF ( audit_row.row_data ? 'id') THEN
			audit_row.row_id = audit_row.row_data->'id';
		END IF;
...
CREATE OR REPLACE FUNCTION audit.set_application_user(userId TEXT) RETURNS void AS $b$
BEGIN
    PERFORM set_config('application_session.user', userId, true);
END;
$b$
LANGUAGE plpgsql
SECURITY DEFINER
...
EXCEPTION WHEN duplicate_schema THEN RAISE NOTICE 'audit schema already exists! auditing should already be enabled, skipping';
END;
$$
